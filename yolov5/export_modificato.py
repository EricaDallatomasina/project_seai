...
def export_saved_model(model,
                       ...
                       conf_thres=0.25,
                       keras=False,
                       custom_type = 'posit160', #NEW
                       prefix=colorstr('TensorFlow SavedModel:')):
        
    # NEW
    if custom_type=='posit160':
        custom_type=tensorflow.posit160
    elif custom_type=='float32':
        custom_type=tensorflow.float32
    elif custom_type=='float64':
        custom_type=tensorflow.float64
    elif custom_type=='float16':
        custom_type=tensorflow.float16
    else:
        print("Error, not valid datatype in export_saved_model (export.py)")
    # END NEW
                       
    # YOLOv5 TensorFlow SavedModel export
    try:
        print("is keras model: ", keras)
        import tensorflow as tf
        from tensorflow.python.framework.convert_to_constants import convert_variables_to_constants_v2

        from models.tf_modificato import TFDetect, TFModel
...
def run(
    ...
    if any((saved_model, pb, tflite, edgetpu, tfjs)):
        ...
        model, f[5] = export_saved_model(model.cpu(),
                                         ...
                                         keras=keras,
                                         custom_type = custom_type) # NEW parameter custom_type
...
def parse_opt():
    ...
    parser.add_argument('--custom_type', type=str, default='posit160', help='target data type')
