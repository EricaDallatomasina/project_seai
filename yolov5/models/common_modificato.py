...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, float_type='posit160'):
        # Usage:
        #   PyTorch:              weights = *.pt
        ...
        if pt:  # PyTorch
            ...
            if saved_model:  # SavedModel
                ...
                if float_type == 'posit160':
                    keras_tf.backend.set_floatx('posit160')
                    
                elif float_type == 'float64':
                    keras_tf.backend.set_floatx('float64')
                
                elif float_type == 'float32':
                    keras_tf.backend.set_floatx('float32')
                    
                elif float_type == 'float16':
                    keras_tf.backend.set_floatx('float16')
                
                else:
                    print("Error: Float Type Not Valid")
                    sys.exit()
    ...
    def forward(self, im, augment=False, visualize=False, val=False):
        ...
        if self.pt:  # PyTorch
            ...
            if self.float_type=='posit160':
                im1 = im.numpy()
                im1 = im1.astype(keras_tf.backend.set_floatx('posit160'))
                im = torch.from_numpy(im1)
            elif self.float_type=='float64':
                im1 = im.numpy()
                im1 = im1.astype(keras_tf.backend.set_floatx('float64'))
                im = torch.from_numpy(im1)
            elif self.float_type=='float16':
                im1 = im.numpy()
    ...
    def model_type(p='path/to/model.pt'):
        ...
        from export_modificato import export_formats
